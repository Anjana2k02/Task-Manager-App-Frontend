...
const AdminManage = () => {
  ...
  useEffect(() => {
    const fetchAdmins = async () => {
      try {
        const data = await getFetcher(enpoints.admin.viewAll);
        setAdmins(data);
      } catch (error) {
  ...
  const downloadPDF = async () => {
    try {
      const response = await getFetcherPramspdf(enpoints.admin.report);
      ...
      <Box sx={{ p: 4, backgroundColor: '#e3f2fd', minHeight: '100vh' }}>
        ...
        >
          ...
          <Typography variant="subtitle1" color="text.secondary" sx={{ mb: 4 }}>
            View and manage admin records with ease.
          </Typography>
                      ...
                      <TableCell>
                        <Box
                          sx={{
                            px: 1.5,
                            py: 0.5,
                            backgroundColor: admin.status === 'Active' ? '#90be6d' : '#f94144',
                            color: '#fff',
                            borderRadius: '12px',
